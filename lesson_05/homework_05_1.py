...
filtered_cars = [
    (car_model, car_description) for car_model, car_description in car_data.items()
    if car_description[1] >= year and car_description[2] >= engine_volume and car_description[4] <= price
]

sorted_cars = sorted(filtered_cars, key=lambda x: x[1][4])

for first_five_cars in sorted_cars[:5]:
